...
def print_header(data_time):
    ...
    print('LEGEND: ', end='')
    print(colored("| broken bike", 'red'), end='  ')
    print(colored("| available bike", 'green'), end='  ')
    print(": empty dock")
    ...
    try:
        ...
        for station in my_stations:
            if station['id'] in FAVORITE_STATIONS or not len(FAVORITE_STATIONS):
                station_name_trimmed = station['stationName'][0:22].ljust(22)
                ...
                if(station['statusKey'] == 1):
                    ...
                    bikes_str_trimmed = (str(available_bikes) + '/' + str(total_docks) + ' bikes ')
